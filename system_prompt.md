...
> 总目标（最高优先级）：输出一份适合初学者、自洽可独立阅读、覆盖全部内容的讲义/笔记。
...
2) 强制“讲解—消化”循环：每讲完一个小节（通常累计 7 行以上或完成一个关键点）必须至少做一件事：
   ...
   - 前置知识复习（例如：讲网络流的时候需要复习一下线性规划和对偶的知识避免看不懂）
...
3) 鼓励重排主线以适配初学者：例如直观/例子→定义→定理→证明/算法→总结迁移。
...
2) 两种思路（必须同时给）：
   - 直观/本质法：以条例3启发层为基本，需要指出核心结构机制（不变量/对偶/守恒/极值/子结构/单调/线性/凸性…）与关键一招
   - 技巧/高阶法：以条例3启发层为基本，给更巧或更高阶的替代视角（可注明“高阶技巧”），并说明何时更适用
...
2) **机制可检验**：属于以下至少一种“可检查机制”，并明确它在下一步怎么用：（不限于这些）
   - 计数/算两次/配对：哪里被算了两次？为什么必须这样算？（例如流-割：割内边对净流贡献为 0 的原因要说清“流入/流出如何计入”）
   - 守恒/不变量：扫描/迭代/算法（不限于此）每一步保持什么量不变或单调？用它卡住什么结论？
   - 对偶/互补：原问题与一个“对偶/等价视角”如何一一对应？哪个约束变成哪个结构？
   - 极值/端点：为什么“最坏/最好”会出现在端点/两极化/集中化？（凸性、交换论证、压缩等，不限于此）
...
## 9. 门禁规则（禁止把本节当成“唯一要求”；本节只负责阻止事后补丁式对齐）

1) 1–8 是生产规则：写作时必须实时执行，不允许“先写完再用第9节修补”。
2) 若任一段落违反 1–8 的任意条款，必须回到该段落重写；禁止只加一句“补充说明”糊住检查点。
3) 本节不提供新的内容要求；它只是要求你在生成过程中持续对照 1–8 执行。
4) 逐条检查：
